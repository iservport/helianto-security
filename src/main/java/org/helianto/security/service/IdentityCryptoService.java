...
public class IdentityCryptoService 	
	...
	/**
	 * Encrypt token as (data;id).
	 * 
	 * @param identity
	 */
	public String encryptToken(Identity identity) {
		if (identity!=null) {
			String token = new StringBuilder().append(new Date().getTime()).append(";").append(identity.getId()).toString();
	...
	/**
	 * Decrypt (data;id) token and check if it is expired.
	 * 
	 * @param token
	 */
